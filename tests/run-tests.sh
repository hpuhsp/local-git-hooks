...
ok "A5 特性分支提交放行" 'git checkout -q -b feature/x && echo y >c.txt && git add -A && git commit -q -m "feat: add c"'
ko "A6 QG_PROTECTED_BRANCHES 自定义保护 develop" 'git checkout -q -b develop && echo z >d.txt && git add -A && export QG_PROTECTED_BRANCHES="main master develop"; git commit -q -m "feat: add d"'
...
w 'fix: bug'
ko "B10 敷衍 fix: bug 被拒" "sh '$CT' '$MSG'"
w 'feat: wip'
ko "B11 敷衍 feat: wip 被拒" "sh '$CT' '$MSG'"
w 'fix: 修改'
ko "B12 敷衍中文 fix: 修改 被拒" "sh '$CT' '$MSG'"
w 'fix: 123'
ko "B13 纯数字描述被拒" "sh '$CT' '$MSG'"
w 'fix: typo'
ok "B14 合理短描述 fix: typo 放行" "sh '$CT' '$MSG'"
w 'fix: bug'
ok "B15 QG_TITLE_QUALITY=0 关闭质量层放行" "QG_TITLE_QUALITY=0 sh '$CT' '$MSG'"
# === AI MODIFIED END ===
...
printf 'a == b\n==== section ====\n========\n' >eq.txt
ok "C3 普通等号不误报" 'git add eq.txt && git commit -q -m "docs: 等号示例不误报"'
...
ok "D1 QG_AI_DEGREE=generated 生成 Generated-by" 'export QG_AI_DEGREE=generated QG_AI_TOOL=claude-code; echo 1 >d1.txt && git add d1.txt && git commit -q -m "feat: add d1" && git log -1 --format=%B | grep -q "^Generated-by: claude-code <ai@noreply.local>"'
ok "D2 AI_TOOL 自带邮箱不重复追加" 'export QG_AI_DEGREE=assisted QG_AI_TOOL="bot <b@x.y>"; echo 2 >d2.txt && git add d2.txt && git commit -q -m "feat: add d2" && git log -1 --format=%B | grep -q "^Assisted-by: bot <b@x.y>$"'
ok "D3 QG-AI 自报行生成 Co-authored-by" 'echo 3 >d3.txt && git add d3.txt && git commit -q -m "feat: add d3" -m "QG-AI: co-authored" && git log -1 --format=%B | grep -q "^Co-authored-by: unknown <ai@noreply.local>$"'
ok "D4 QG-AI 行已从正文清除" '! git log -1 --format=%B | grep -q "^QG-AI:"'
ok "D5 默认 none 不写 AI trailer" 'echo 5 >d5.txt && git add d5.txt && git commit -q -m "feat: add d5" && ! git log -1 --format=%B | grep -qE "^(Assisted|Generated|Co-authored)-by:"'
ok "D6 未知 QG_AI_DEGREE 忽略并放行" 'export QG_AI_DEGREE=bogus; echo 6 >d6.txt && git add d6.txt && git commit -q -m "feat: add d6" && ! git log -1 --format=%B | grep -qE "^(Assisted|Generated|Co-authored)-by:"'
ok "D7 模板注释行已清理" '! git log -1 --format=%B | grep -qE "QG-AI|^# ─"'
ok "D8 -s 签名不重复追加" 'echo 8 >d8.txt && git add d8.txt && git commit -q -s -m "feat: add d8" && [ "$(git log -1 --format=%B | grep -c "^Signed-off-by:")" = "1" ]'
...
exit 1
EOF
ko "G1 pre-commit 栈叠加脚本失败可阻断" 'echo g >g.txt && git add g.txt && git commit -q -m "feat: add g"'
rm -f .githooks/stacks/java/pre-commit.d/block.sh
ok "G2 移除叠加脚本后放行" 'git commit -q -m "feat: add g"'
