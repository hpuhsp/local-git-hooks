...
把这套钩子搬进**另一个已在开发的项目**时，**不动业务代码**，只做加法。所有命令都在**目标项目根目录**（有 `.git` 的那层）执行。

**第 1 步 · 拷入文件**

| 必带（核心逻辑）                                              | 可选（按需启用 / 合并几行）                                             |
| ------------------------------------------------------------- | ----------------------------------------------------------------------- |
| `.githooks/`（整个目录）                                      | `scripts/setup.*`、`Makefile`（仅安装便捷脚本，可不要）                 |
| `lefthook.yml`（根目录配置）                                  | `commitlint.config.js`、`.prettierignore`（启用标题规范 / prettier 时） |
| `.gitattributes` 里的 LF 规则（无则新建，有则**并入**这几行） | `.gitignore` 的 secrets 段（**并入**几行）                              |
...
- **JS 栈（已有 `package.json`）**：并入 devDependencies（`lefthook`，及可选 `@commitlint/*`、`prettier`）+ 一行脚本 `"prepare": "lefthook install || true"`。
- **非 JS 栈（Java / Python，无 `package.json`）**：**不加 `package.json`**，直接装二进制：`brew install lefthook` / `scoop install lefthook` / `winget install evilmartians.lefthook`。

**第 3 步 · 激活并提交**

```bash
make setup        # 或 sh scripts/setup.sh / npx lefthook install
git add .githooks lefthook.yml .gitattributes   # 连同上面拷入的文件
git commit -m "chore: 接入本地 git hooks"
```

提交后，团队每位成员拉到这些文件，各自跑一次 `make setup`（或 `npm install`）即生效。

> **侵入面**：本质是「1 个目录 `.githooks/` + 1 个根配置 `lefthook.yml` + 几行合并」。钩子激活后落在 `.git/hooks`（不入库）。唯一可能冲突点：项目已用 Husky 时 `core.hooksPath` 被占用，`setup` 会检测并提示先 `git config --unset core.hooksPath`。
